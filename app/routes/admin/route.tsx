...
const firebaseConfig = {
  ...
  databaseURL:
    "https://wedding-38b49-default-rtdb.asia-southeast1.firebasedatabase.app",
...
export const meta: MetaFunction = () => {
  return [
    { title: "Admin Centre" },
    { name: "description", content: "Admin Centre" },
  ]
}
...
export async function clientLoader() {
  ...
  users.forEach(async (user) => {
    const subCollection = await getDocs(
      collection(db, "users", user.id, "users")
    )
  ...
  const total = {
    ...
    rsvp: users.reduce(
      (acc, user) => acc + (user.rsvp !== "awaiting" ? 1 : 0),
      0
    ),
  }
        ...
        <Card shadow="xs" padding="xl">
          {access ? (
              ...
              <Table striped withTableBorder stickyHeader highlightOnHover>
                ...
                <Table.Tbody>
                  ...
                  <Table.Tr>
                    <Table.Td></Table.Td>
                    <Table.Td>
                      {Object.keys(loaderData?.users).length -
                        loaderData?.total.rsvp}
                    </Table.Td>
            ...
            <Stack>
              ...
              <MantineButton
                type="submit"
                onClick={() => password === "beanjuice" && setAccess(true)}
              >
                Submit
              </MantineButton>
            </Stack>
